...
async function authInscription(req: Request, res: Response) {
  ...
  if (!courriel || !motDePasse || !nom || !prenom) {
    throw new Error(
      "Ces informations sont requises: courriel, mot de passe, nom, prenom.",
    );
  }
  ...
  if (user) {
    res
      ...
      .json({
        message:
          "Un compte existe avec ce courriel.  Veuillez choisir une autre adresse.",
      });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ id: user.courriel, role: user.role, cree: user.createdAt });
  } catch (error) {
...
async function authConnexion(req: Request, res: Response) {
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      res.status(400).json({
        message:
          "La connexion est impossible.  Veuillez valider le courriel et mot de passe.",
      });
    }
  }
}
